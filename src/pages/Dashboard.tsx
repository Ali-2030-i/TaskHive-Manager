      ...
      <div className="pt-28 pb-16 min-h-screen bg-gradient-to-br from-background via-background to-secondary/5">
        <div className="container mx-auto px-6">
          ...
          {isLoading ? (
            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
              {Array.from({ length: 4 }).map((_, i) => (
                <div key={i} className="bg-card border border-border/40 rounded-2xl p-6 space-y-4 animate-pulse">
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
              {stats.map((stat, index) => (
                <div
                  key={stat.label}
                  className={cn(
                    "group relative overflow-hidden rounded-2xl border p-6 transition-all duration-300 hover:shadow-lg hover:-translate-y-1 animate-slide-up-in",
                ...
                >
                  ...
                  <div className="relative">
                    <div className="flex items-start justify-between mb-4">
                      <div className={cn("p-3 rounded-xl transition-all duration-300 group-hover:scale-110", stat.bgColor)}>
                        <stat.icon className={cn("w-6 h-6", stat.color)} />
                      </div>
                      <ArrowUpRight className={cn("w-5 h-5 opacity-0 group-hover:opacity-100 transition-all duration-300 text-primary", stat.color)} />
                    </div>
          ...
          <div className="grid lg:grid-cols-3 gap-6">
            {/* Progress Chart */}
            <div className="lg:col-span-2 bg-gradient-to-br from-background/40 to-background/20 border border-border/40 rounded-2xl p-8 transition-all duration-300 hover:border-primary/30 hover:shadow-lg group">
            ...
            {/* Recent Activity */}
            <div className="bg-gradient-to-br from-background/40 to-background/20 border border-border/40 rounded-2xl p-8 transition-all duration-300 hover:border-primary/30 hover:shadow-lg group">
              <div className="flex items-center gap-2 mb-8">
              ...
              <div className="space-y-4">
                {activities.slice(0, 5).map((activity, idx) => (
                  ...
                  >
                    <div className="w-2 h-2 rounded-full bg-primary mt-2 shrink-0 group-hover/activity:scale-150 group-hover/activity:shadow-lg transition-all duration-300" />
